...
void neuron_serialize(FILE *fp, N neural_net) {
	...
	sprintf(buf, "neuron[%s]\n", MATRIX_DESC_FMT_STR);

	fprintf(fp, buf, neural_net->layers);

	UINT i;
	for(i = 0; i < neural_net->layers; ++i){
		matrix_serialize(fp, neural_net->weights[i]);
		if(i != neural_net->layers - 1) 
			fprintf(fp, "\n");
	}
...
N neuron_deserialize(FILE *fp) {
	...
	sprintf(buf, "neuron[%s]", MATRIX_DESC_FMT_STR);

	fscanf(fp, buf, &layers);

	N neural_net = malloc(sizeof(*neural_net));

	neural_net->layers = layers;

	neural_net->weights = malloc(sizeof(*neural_net->weights)*layers);

	UINT i;
	for(i = 0; i < layers; i++) {
neural_net->weights[i] = matrix_deserialize(fp);
	}
	return neural_net;
}
...
UINT neuron_eq(N a, N b) {
	...
	if((a->layers != b->layers)) return 0;

	UINT i;
	for(i = 0; i < a->layers; ++i) {

		if(!matrix_eq(a->weights[i], b->weights[i])){
			return 0;
		}
	}
